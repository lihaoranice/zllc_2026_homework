...
/**
  * @brief  电机测试函数（逐个测试电机转向）
  * @param  无
  * @retval 无
  * @note   用于验证每个电机的正反转是否正确
  */
void Motor_Test_Individual(void)
{
	extern int printf(const char *fmt, ...);
	uint16_t test_speed = 300;
	
	printf("\r\n===== 电机测试开始 =====\r\n");
	
	// 测试左前轮 (LF)
	printf("[测试] 左前轮(LF) 正转...\r\n");
	Mecanum_Control(test_speed, 0, 0, 0);
	HAL_Delay(2000);
	Car_Stop();
	HAL_Delay(500);
	...
	// 测试右前轮 (RF)
	printf("[测试] 右前轮(RF) 正转...\r\n");
	Mecanum_Control(0, test_speed, 0, 0);
	HAL_Delay(2000);
	Car_Stop();
	HAL_Delay(500);
	...
	// 测试左后轮 (LB)
	printf("[测试] 左后轮(LB) 正转...\r\n");
	Mecanum_Control(0, 0, test_speed, 0);
	HAL_Delay(2000);
	Car_Stop();
	HAL_Delay(500);
	...
	// 测试右后轮 (RB)
	printf("[测试] 右后轮(RB) 正转...\r\n");
	Mecanum_Control(0, 0, 0, test_speed);
	HAL_Delay(2000);
	Car_Stop();
	HAL_Delay(500);
