...
// 舵机当前角度 (私有变量)
static uint8_t servo_angles[4] = {90, 90, 90, 90};
...
void Servo_Init(void)
{
	...
	printf("[Servo] Initialized, all servos at 90 degrees\r\n");
}
...
void Servo_SetAngle(Servo_ID_t servo_id, uint8_t angle)
{
	...
	// printf("[Servo%d] Angle: %d\u00b0 (Pulse: %dus)\r\n", servo_id+1, angle, pulse_us);
}
...
void Servo_SetPulse(Servo_ID_t servo_id, uint16_t pulse_us)
{
	...
	switch(servo_id)
	{
		case SERVO_1: channel = TIM_CHANNEL_1; break;
		case SERVO_2: channel = TIM_CHANNEL_2; break;
		case SERVO_3: channel = TIM_CHANNEL_3; break;
		case SERVO_4: channel = TIM_CHANNEL_4; break;
		default: return;
	}
	
	// 设置PWM比较值 (脉宽直接对应计数值,因为定时器频率是1MHz)
	__HAL_TIM_SET_COMPARE(&htim4, channel, pulse_us);
}
...
void PS2_Control_Servo(uint8_t btn1, uint8_t btn2)
{
	...
	else if(!(btn1 & BTN_DOWN))  // DOWN键按下,舵机1反转(角度减少)
	{
		uint8_t current_angle = servo_angles[SERVO_1];
	...
	else if(!(btn1 & BTN_RIGHT))  // RIGHT键按下,舵机2反转(角度减少)
	{
		uint8_t current_angle = servo_angles[SERVO_2];
		if(current_angle >= SERVO_STEP_ANGLE)
		{
	...
	else if(!(btn2 & BTN_CRO))  // ×键按下,舵机3反转(角度减少)
	{
		uint8_t current_angle = servo_angles[SERVO_3];
		if(current_angle >= SERVO_STEP_ANGLE)
		{
	...
	else if(!(btn2 & BTN_CIR))  // ○键按下,舵机4反转(角度减少)
	{
		uint8_t current_angle = servo_angles[SERVO_4];
		if(current_angle >= SERVO_STEP_ANGLE)
		{
