...
void UART_Process_Command(void)
{
    ...
    else if(strcmp(token, "MOVE_TURN") == 0)
    {
        // 格式：CMD:MOVE_TURN,<mm>,<move_speed>,<dir(L|R)>,<turn_ms>,<turn_speed>
        char *mm_str = strtok(NULL, ",");
        char *move_speed_str = strtok(NULL, ",");
        char *dir_str = strtok(NULL, ",");
        char *turn_ms_str = strtok(NULL, ",");
        char *turn_speed_str = strtok(NULL, ",");
        
        if(mm_str == NULL || move_speed_str == NULL || dir_str == NULL || turn_ms_str == NULL || turn_speed_str == NULL)
        {
            printf("ERROR: Invalid MOVE_TURN command\r\n");
            return;
        }
        
        uint32_t mm = (uint32_t)atoi(mm_str);
        int16_t move_speed = (int16_t)atoi(move_speed_str);
        char dir = dir_str[0];
        uint32_t turn_ms = (uint32_t)atoi(turn_ms_str);
        int16_t turn_speed = (int16_t)atoi(turn_speed_str);
        
        if(dir != 'L' && dir != 'R' && dir != 'l' && dir != 'r')
        {
            printf("ERROR: Invalid turn dir (L/R)\r\n");
            return;
        }
        if(dir == 'l') dir = 'L';
        if(dir == 'r') dir = 'R';
        ...
        // 启动序列动作（在主循环中执行）
        Start_MoveThenTurn(mm, move_speed, dir, turn_ms, turn_speed);
    }
    else
    {
